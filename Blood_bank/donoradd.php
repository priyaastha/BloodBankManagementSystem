...
<?php
    ...
    if(isset($_POST['register'])){
        ...
        //query to retrieve data from donors table with email same as that in form 
        $duplicate="select * from donors where Email='$email'";

        //to execute the query
        $q=mysqli_query($connect,$duplicate);

        //to count no. of rows for that query
        $r=mysqli_num_rows($q);
        
        //if there exists such an email, give an error message
        if($r>0){
            echo "  <html>
                        <body style='float:top;'>
                            <div class='alert alert-danger alert-dismissible'>
                                <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
                                <strong>Warning!</strong> Donor Already exists with the given email.
                            </div>
                        </body>
                    </html>";
        }

        //if no such email already exists, add data in table and show success message
        else{
...
?>
    ...
    <div class="container" >
        <div class="col-md-12">
            ...
            <form action="donoradd.php" method="post" style="margin-left: 240px;margin-right:80px;width: 100%;">
                <div class ="row">
                    <div class="col-md-6">
                        <div class="col-md-10" >
                            ...
                            <button type="submit" class="btn" name="register" style='margin-left:130px;'>Add Donor</button>
                        </div>
                    </div>
                </div>
            </form>
