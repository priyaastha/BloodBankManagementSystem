...
<?php
    ...
    //retrieve data from donations table in db
    $check='select * from donations';
    
    //to execute the query
    $q=mysqli_query($connect,$check);

        //to print the table
        echo "  <table id='myTable' class='table table-striped table-bordered' style='width:80%;margin-top:-290px;'>
        ...
        while($row=mysqli_fetch_array($q)){
            echo("<tr style='font-size:18px;'><th>" .$row['Donationid']."</th><th>".$row['Name']."</th><th>".$row['Bloodgroup']."</th><th>".$row['Date']."</th><th>".$row['City']."</th><th>".$row['Pincode'].'</th><br><br>');
        }
        //table closed
        echo "</table>";
?>
